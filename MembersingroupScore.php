...
// WHERE  groups.GroupName="jij"
 //$sql = "SELECT * FROM `user` inner join groups on user.GroupId=groups.GroupId WHERE groups.GroupName="."'$group1'";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    echo  nl2br ( ";".$row["Username"].',' . $row["Role"].',' . $row["Rank"].',' . $row["Status"] .',' . $row["User_Id"].',' . $row["Score_Elec"].',' . $row["Score_Fire"].',' . $row["Score_Transfer"].',' . $row["Score_total"].',' . $row["GroupId"]);
  }
} else {
  echo "0 results";
}
